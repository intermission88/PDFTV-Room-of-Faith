...
- Alur News: writer submit (`writer_submit_news`, selalu `pending`) → CEO setujui/tolak/tarik (`ceo_set_news_status`). **RLS hanya membuka `status='approved'`**; daftar pending dibaca lewat RPC `SECURITY DEFINER`, bukan select langsung. Login memakai satu modal dengan rantai verifikasi `verify_admin` → `verify_moderator` → `verify_writer` → `verify_ceo`.
...
- Artikel News ikut di-pre-render ke `news/p/<id>/` (hanya yang `approved`), tapi **tanpa PNG**: `og:image` memakai `cover_url` artikel (cadangan `assets/og/fallback.png`), jadi tidak ada churn font. Kalau tabel `PDFTV News` belum ada, skrip hanya memberi warning dan feed tetap diproses.
