...
-- ============================================================
-- 8. STATISTIK PENGUNJUNG + RINGKASAN PLATFORM
--
-- Pengunjung dihitung UNIK sepanjang masa: hash IP dengan salt rahasia.
-- Yang disimpan hanya hash (bukan IP mentah), dan salt-nya ada di schema
-- `private` supaya hash tidak bisa dibalik lewat tabel IP yang sudah jadi.
--
-- Tabel visit sengaja TANPA policy RLS: anon tidak boleh membaca daftar
-- pengunjung. Angka totalnya hanya keluar lewat public.get_platform_stats().
-- ============================================================
CREATE TABLE IF NOT EXISTS private.site_config (
    key   TEXT PRIMARY KEY,
    value TEXT NOT NULL
);
REVOKE ALL ON TABLE private.site_config FROM anon, authenticated;
ALTER TABLE private.site_config ENABLE ROW LEVEL SECURITY;

-- Salt dibuat sekali lalu dipakai seterusnya. Kalau salt berubah, semua
-- pengunjung akan terhitung sebagai orang baru.
CREATE OR REPLACE FUNCTION private.visitor_salt()
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = private, extensions
...
DECLARE
    s TEXT;
BEGIN
    SELECT value INTO s FROM private.site_config WHERE key = 'visitor_salt';
    IF s IS NULL THEN
        s := encode(gen_random_bytes(32), 'hex');
        INSERT INTO private.site_config (key, value)
        VALUES ('visitor_salt', s)
...
END;
$$;
REVOKE ALL ON FUNCTION private.visitor_salt() FROM anon, authenticated;

CREATE TABLE IF NOT EXISTS public."PDFTV Visits" (
    id           BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    visitor_hash TEXT NOT NULL UNIQUE,
    first_seen   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    last_seen    TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
ALTER TABLE public."PDFTV Visits" ENABLE ROW LEVEL SECURITY;
-- Tidak ada policy apa pun: baca/tulis langsung dari anon selalu ditolak.

-- Catat satu kunjungan. Dipanggil halaman landing, sekali per sesi tab.
...
DECLARE
    ...
    v_hash    TEXT;
BEGIN
    client_ip := btrim(COALESCE(
        split_part(current_setting('request.headers', true)::json->>'x-forwarded-for', ',', 1),
        ''
    ));

    -- Tanpa header IP (mis. dipanggil dari SQL Editor) tidak ada yang dicatat
    IF client_ip = '' THEN
        RETURN;
    END IF;

    v_hash := encode(digest(client_ip || private.visitor_salt(), 'sha256'), 'hex');

    INSERT INTO public."PDFTV Visits" (visitor_hash)
    VALUES (v_hash)
    ON CONFLICT (visitor_hash) DO UPDATE SET last_seen = NOW();
END;
$$;

-- Semua angka statistik dalam SATU panggilan (hemat round-trip).
...
AS $$
    SELECT jsonb_build_object(
        'visitors',    (SELECT COUNT(*) FROM public."PDFTV Visits"),
        'confessions', (SELECT COUNT(*) FROM public."PDFTV Feeds"),
        'upvotes',     (SELECT COALESCE(SUM(upvotes), 0) FROM public."PDFTV Feeds"),
        'comments',    (SELECT COALESCE(SUM(
                            CASE WHEN jsonb_typeof(comments) = 'array'
                                 THEN jsonb_array_length(comments) ELSE 0 END), 0)
                        FROM public."PDFTV Feeds"),
        'blackjack_total', (SELECT COALESCE(SUM(streak_count), 0) FROM public."PDFTV Blackjack Leaderboard"),
        'blackjack_best',  (SELECT COALESCE(MAX(streak_count), 0) FROM public."PDFTV Blackjack Leaderboard")
    );
$$;

-- ============================================================
-- 9. PERKETAT RLS FEEDS
...
-- ============================================================
-- 10. PASSWORD ADMIN & MODERATOR (SETEL DI LUAR REPO)
...
-- ============================================================
-- 11. NEWS: ARTIKEL + DASHBOARD WRITER & CEO
-- Sub halaman terpisah dari Room of Faith. Semua penulisan lewat RPC;
-- publik hanya bisa MEMBACA artikel yang sudah disetujui CEO
-- (status = 'approved'). Artikel pending/rejected hanya terjangkau
-- lewat RPC SECURITY DEFINER yang memverifikasi password.
-- ============================================================
CREATE TABLE IF NOT EXISTS public."PDFTV News" (
    id            BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    title         TEXT NOT NULL,
    excerpt       TEXT,
    body          TEXT NOT NULL,
...
ALTER TABLE public."PDFTV News" ENABLE ROW LEVEL SECURITY;

-- Tidak ada policy insert/update/delete: semua tulis WAJIB lewat RPC.
DROP POLICY IF EXISTS "Public read approved news" ON public."PDFTV News";
CREATE POLICY "Public read approved news" ON public."PDFTV News"
    FOR SELECT USING (status = 'approved');

-- --- Verifikasi kredensial role baru (pola verify_moderator) ---
CREATE OR REPLACE FUNCTION public.verify_writer(p_pass TEXT)
RETURNS BOOLEAN
LANGUAGE sql
SECURITY DEFINER
SET search_path = private, public, extensions
AS $$
    SELECT private.verify_credential('writer', p_pass);
$$;

CREATE OR REPLACE FUNCTION public.verify_ceo(p_pass TEXT)
RETURNS BOOLEAN
LANGUAGE sql
SECURITY DEFINER
SET search_path = private, public, extensions
AS $$
    SELECT private.verify_credential('ceo', p_pass);
$$;
...
BEGIN
    ...
    IF char_length(v_title) < 5 OR char_length(v_title) > 160 THEN
        RAISE EXCEPTION 'judul harus 5-160 karakter';
    END IF;
    IF char_length(v_body) < 20 OR char_length(v_body) > 20000 THEN
        RAISE EXCEPTION 'isi artikel harus 20-20000 karakter';
    END IF;
    IF v_cover IS NOT NULL AND (char_length(v_cover) > 500 OR v_cover !~* '^https?://') THEN
        RAISE EXCEPTION 'URL cover tidak valid (harus diawali http/https)';
    END IF;
    IF v_cat IS NOT NULL AND char_length(v_cat) > 40 THEN
        RAISE EXCEPTION 'kategori terlalu panjang';
    END IF;
...
CREATE OR REPLACE FUNCTION public.writer_update_news(
    p_pass TEXT,
    p_id BIGINT,
    p_title TEXT,
    p_body TEXT,
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'artikel tidak bisa diedit (sudah ditinjau CEO)';
    END IF;
END;
$$;

CREATE OR REPLACE FUNCTION public.writer_delete_news(p_pass TEXT, p_id BIGINT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    ...
    DELETE FROM public."PDFTV News"
        WHERE id = p_id
          AND status IN ('pending', 'rejected');

    IF NOT FOUND THEN
        RAISE EXCEPTION 'hanya artikel pending/ditolak yang bisa dihapus';
    END IF;
END;
$$;

CREATE OR REPLACE FUNCTION public.writer_list_news(p_pass TEXT)
RETURNS SETOF public."PDFTV News"
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
    ...
    RETURN QUERY
        SELECT * FROM public."PDFTV News" ORDER BY created_at DESC;
END;
$$;

-- --- Aksi CEO: daftar + set status + hapus ---
CREATE OR REPLACE FUNCTION public.ceo_list_news(p_pass TEXT, p_status TEXT DEFAULT NULL)
RETURNS SETOF public."PDFTV News"
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    IF NOT public.verify_ceo(p_pass) THEN
        RAISE EXCEPTION 'akses CEO ditolak';
    END IF;

    IF v_status IS NOT NULL
       AND v_status NOT IN ('pending', 'approved', 'rejected', 'unpublished') THEN
        RAISE EXCEPTION 'status tidak dikenal';
    END IF;

    RETURN QUERY
        SELECT * FROM public."PDFTV News"
        WHERE v_status IS NULL OR status = v_status
        ORDER BY created_at DESC;
END;
$$;

CREATE OR REPLACE FUNCTION public.ceo_set_news_status(
    p_pass TEXT,
    p_id BIGINT,
    p_status TEXT,
...
BEGIN
    ...
    IF v_status NOT IN ('pending', 'approved', 'rejected', 'unpublished') THEN
        RAISE EXCEPTION 'status tidak dikenal';
    END IF;
    IF char_length(COALESCE(v_reason, '')) > 500 THEN
        RAISE EXCEPTION 'alasan penolakan maksimal 500 karakter';
    END IF;
...
CREATE OR REPLACE FUNCTION public.ceo_delete_news(p_pass TEXT, p_id BIGINT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    ...
    DELETE FROM public."PDFTV News" WHERE id = p_id;
...
-- Jumlah artikel tayang & menunggu (untuk widget statistik).
...
AS $$
    SELECT jsonb_build_object(
        'published', (SELECT COUNT(*) FROM public."PDFTV News" WHERE status = 'approved'),
        'pending',   (SELECT COUNT(*) FROM public."PDFTV News" WHERE status = 'pending')
    );
$$;
...
GRANT EXECUTE ON FUNCTION public.ceo_set_news_status(TEXT, BIGINT, TEXT, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.ceo_delete_news(TEXT, BIGINT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_news_stats() TO anon, authenticated;

-- Kredensial writer & CEO (setel di luar repo, sama seperti admin/moderator):
--   SELECT private.set_credential('writer', '<password writer>');
