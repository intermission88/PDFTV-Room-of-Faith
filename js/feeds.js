...
function renderFeeds() {
    ...
    container.innerHTML = filtered.map(item => {
        ...
        return `
            <div class="glass p-3.5 rounded-2xl border ${isPinned ? 'border-amber-500/60 bg-amber-950/20 shadow-amber-500/10' : 'border-white/10 hover:border-emerald-500/30'} shadow-xl space-y-2 relative overflow-hidden transition-all text-left w-full">
                ${isPinned ? `
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2.5 text-left">
                        <div class="w-8 h-8 rounded-xl bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center text-xs font-bold text-emerald-300 font-mono-custom flex-shrink-0">
                            ${firstLetter}
                        </div>
                        <div class="text-left">
                ...
                <p class="text-xs text-slate-100 leading-relaxed font-sans whitespace-pre-wrap text-left break-words">
                    ${escapeHtml(item.confession)}
                </p>

                <div class="flex items-center gap-2 pt-1.5 border-t border-white/10 text-[11px] font-mono-custom font-bold">
                    <button onclick="upvoteFeed('${item.id}')" class="flex items-center gap-1.5 px-3 py-1 rounded-xl ${hasUpvoted ? 'bg-amber-500/25 text-amber-300 border border-amber-500/50' : 'bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 hover:text-amber-400'} transition active:scale-95">
                        <span>🔥</span>
                        <span>${item.upvotes}</span>
                        <span class="text-[9px] opacity-75">${hasUpvoted ? 'Liked' : 'Upvote'}</span>
                    </button>
                    <button onclick="toggleCommentsDrawer('${item.id}')" class="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 hover:text-white transition active:scale-95">
                        <span>💬</span>
                ...
                <div id="comments-drawer-${item.id}" class="${drawerOpen ? '' : 'hidden'} pt-2 border-t border-white/10 space-y-2 text-left">
                    <div class="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                        ${commentsList.length === 0
                            ? `<p class="text-[10px] text-slate-500 italic py-1 text-left font-mono-custom">Belum ada komentar.</p>`
                            : commentsList.map(c => `
                                <div class="bg-black/40 border border-white/10 p-2 rounded-xl text-[11px] text-slate-200 font-sans leading-relaxed text-left">
                                    💬 ${escapeHtml(c)}
                                </div>
                            `).join('')
                        }
                    ...
                    <form onsubmit="addCommentToFeed('${item.id}', event)" class="flex gap-1.5 pt-1">
                        <input id="comment-input-${item.id}" type="text" placeholder="Tulis tanggapan..." required
                            class="flex-1 bg-black/60 border border-white/10 rounded-xl px-2.5 py-1.5 text-[11px] text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500/50 font-sans text-left">
                        <button type="submit" class="px-3 py-1.5 bg-emerald-500/20 hover:bg-emerald-500/40 text-emerald-300 border border-emerald-500/40 rounded-xl text-[10px] font-mono-custom font-bold transition active:scale-95">
                            Kirim
                        </button>
                    </form>
                </div>
